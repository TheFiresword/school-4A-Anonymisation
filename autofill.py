...
def createJoin(fichierorigin, fichiervictim, nomjointure):
    ...
    nn.fit(origin[['lont', 'lat']])

    distances, indices = nn.kneighbors(victim[['lont', 'lat']])

    victim['Indice_Original'] = indices

    resultado_join = pd.merge(victim, origin, left_on='Indice_Original', right_index=True, suffixes=('victim', 'origin'))

    resultado_final = resultado_join[resultado_join['datevictim'] == resultado_join['dateorigin']]
    resultado_final  = resultado_final [['id_o', 'dateorigin', 'lontorigin', 'latorigin', 'id_x']]
    resultado_final .columns = ['id_o', 'date', 'lont', 'lat', 'id_x']


    #jointure = origin.merge(victim, on=['date', 'lont', 'lat'])
    resultado_final .to_csv(nomjointure + ".csv", index=False)
