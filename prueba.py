...
def FilterCommonIdx(fichierjointure, nomjointure):
        ...
        # Agrupar por 'id_o' y 'date' y encontrar el 'id_x' más común en cada grupo
        most_common_id_x = df.groupby(['id_o', 'date'])['id_x'].transform(lambda x: x.mode().iloc[0])
        
        # Agregar la columna 'most_common_id_x' al DataFrame original
        df['most_common_id_x'] = most_common_id_x
        
        # Filtrar para mantener solo las filas donde 'id_x' coincide con 'most_common_id_x'
        registros_finales = df[df['id_x'] == df['most_common_id_x']]
        
        # Eliminar duplicados basados en 'id_o' y 'date'
...
def FilterUncommonIdx(fichierjointure, nomjointure):
    df = pd.read_csv(fichierjointure)

    # Agrupar por 'id_o' y 'date' y encontrar el 'id_x' menos común en cada grupo
    least_common_id_x = df.groupby(['id_o', 'date'])['id_x'].transform(lambda x: x.mode().iloc[-1])

    # Agregar la columna 'least_common_id_x' al DataFrame original
    df['least_common_id_x'] = least_common_id_x

    # Filtrar para mantener solo las filas donde 'id_x' coincide con 'least_common_id_x'
    registros_finales = df[df['id_x'] == df['least_common_id_x']]
